...
if __name__ == '__main__':
    frequency_path = '/behavior_baseline/baseline/theia/data/frequency_train_data.csv'

    for file in event_csv_list:
        print('Processing:', file)
        if file == '/Datasets/theia/e3_theia_preparation_event_12.csv':
            cnt = 0
            with open(file, 'r') as f:
                for line in f:
                    cnt += 1
                    if cnt >= 3034800 and cnt <= 3295700:
                        continue
        ...
        if file == '/Datasets/theia/e3_theia_preparation_event_20.csv':
            cnt = 0
            with open(file, 'r') as f:
                for line in f:
                    cnt += 1
                    if cnt >= 1017500 and cnt <= 1234500:
                        continue
                    event_frequency[line] += 1
                    subject = line.rsplit(', ', 1)[0]
                    subject_frequency[subject] += 1
