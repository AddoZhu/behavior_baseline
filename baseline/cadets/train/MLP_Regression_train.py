...
def train_model(events, values, mlp_model_path):
    ...
    model.add(tf.keras.layers.Input(shape=(events.shape[1],)))
    ...
    model.fit(
        ...
        callbacks=[
            keras.callbacks.EarlyStopping(monitor='val_loss', patience=20, min_delta=0.001, restore_best_weights=True)
            ]
    )
    end_time = time.time()
    training_time = end_time - start_time
    print(f"MLP model training time: {training_time:.2f} 秒")
...
if __name__ == '__main__':
    ...
    mlp_model_path = f'/behavior_baseline/baseline/cadets/model/{embedding_model}_mlp_model.keras'
    ...
    train_model(events, values, mlp_model_path)